...
#include "hcsr501.h"
/*
hcsr501红外传感器的输出引脚接A0
蓝牙的TXD接A10 RXD接A9
舵机的信号传输引脚接A1
OLED的SCL接B10 SDA接B11
*/
/*
此程序的控制优先级为蓝牙>红外传感器
*/
...
uint32_t state;//舵机状态
uint8_t B_state;//蓝牙状态
uint8_t hcsr_state;//红外传感武器状态
uint8_t pre_hcsr_state;//红外传感器的上一个状态
uint8_t arr_rx1;
int main(void)
{
	HAL_Init();
...
//		}
		...
		if(pre_hcsr_state!=hcsr_state){
			__HAL_TIM_SET_COMPARE(&TIM_SG90HandInit,TIM_CHANNEL_2,state);
			if(hcsr_state==1){
				HAL_UART_Transmit(&UART_HandInit,"已打开\r\n",10,1000);
				while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
			}else
			{
				HAL_UART_Transmit(&UART_HandInit,"已关闭\r\n",10,1000);
				while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
			}
		}
	}
