...
//	CAN_ITConfig(CAN1,CAN_IT_FMP0,ENABLE);
	...
	CAN_LoopTestInit.CAN_Mode=CAN_Mode_Normal;//环回测试模式
	...
	CAN_LoopTestFilterInit.CAN_FilterIdHigh=0x123<<5;
	CAN_LoopTestFilterInit.CAN_FilterIdLow=(0x234<<5)|0x10;
	CAN_LoopTestFilterInit.CAN_FilterMaskIdHigh=(0x456<<5)|0X10;
	CAN_LoopTestFilterInit.CAN_FilterMaskIdLow=0x2FE<<5;
...
void CANLoopTest_Send(CanTxMsg* CAN_LoopTest){
	...
	MYOLEDPLUS_ShowHexNum(30,50,Num,2,6);
	uint32_t TimeOut=0;
...
uint8_t Receive_Flag(void){
	if(CAN_MessagePending(CAN1,CAN_FIFO0)==SET){
		RxFlage=1;
	}
	return RxFlage;
}
