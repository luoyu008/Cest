...
void DCmotors_Init(void){
	...
	TIM_TimeBaseInitStructure.TIM_Period=100-1;//记数周期即ARR自动装填器的值
...
void Direction(uint8_t in1,uint8_t in2){
	GPIO_WriteBit(GPIOA,GPIO_Pin_5,(BitAction)in1);
	GPIO_WriteBit(GPIOA,GPIO_Pin_4,(BitAction)in2);
}
