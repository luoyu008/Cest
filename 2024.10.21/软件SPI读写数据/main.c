...
uint8_t MID;
uint16_t ID;
uint8_t DataWrite[]={0x55,0x66,0x77,0x88};
uint8_t DataRead[]={0,0,0,0};
int main()
{	
	OLED_Init();
	W25Q64ruanjian_Init();
	W25Q64ruanjian_ReadID(&MID,&ID);
	W25Q64ruanjian_SectorErase(0x000000);
	W25Q64ruanjian_WritePage(0x0000000,DataWrite,4);
	W25Q64ruanjian_ReadData(0x000000,DataRead,4);
	while(1){
