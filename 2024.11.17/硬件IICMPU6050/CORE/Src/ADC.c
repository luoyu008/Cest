...
void ADC_ccscmode(void){
	...
	DMA_ccscmodeHandInit.Init.MemInc=DMA_MINC_ENABLE;//内存地址自增
	...
	ADC_ccscmodeHandInit.Init.NbrOfConversion=4;//3个通道
	...
	HAL_ADC_Start_DMA(&ADC_ccscmodeHandInit,(uint32_t*)data,4);
}
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* hadc){
	if(hadc->Instance==ADC1){
		...
		GPIO_Init.Pin=GPIO_PIN_4|GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_2;
