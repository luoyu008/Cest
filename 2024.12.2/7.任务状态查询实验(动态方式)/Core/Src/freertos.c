...
void EntryTask2(void const * argument)
{
	...
	char str[50];
	TaskStatus_t * taskstate=malloc(sizeof(TaskStatus_t));
	uint8_t Count2=uxTaskGetSystemState(taskstate, Count,NULL);

//	for(uint8_t i=0;i<Count2;i++){
//	HAL_UART_Transmit(&UART_HandInit,taskstate[i].pcTaskName,10,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//	
//	HAL_UART_Transmit(&UART_HandInit,"\t",1,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//		
//	sprintf(str,"%ld",taskstate[i].uxBasePriority);
//	HAL_UART_Transmit(&UART_HandInit,str,1,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//		
//	HAL_UART_Transmit(&UART_HandInit,"\t",1,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//		
//	sprintf(str,"%ld",taskstate[i].uxCurrentPriority);
//	HAL_UART_Transmit(&UART_HandInit,str,1,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//		
//	HAL_UART_Transmit(&UART_HandInit,"\t",1,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);	
//	
//	sprintf(str,"%ld",taskstate[i].xTaskNumber);
//	HAL_UART_Transmit(&UART_HandInit,str,1,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//	
//	HAL_UART_Transmit(&UART_HandInit,"\t",1,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//	
//	sprintf(str,"%u",taskstate[i].usStackHighWaterMark);
//	HAL_UART_Transmit(&UART_HandInit,str,3,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//		
//	HAL_UART_Transmit(&UART_HandInit,"\r\n",2,100);
//	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
//	}
	...
	vTaskGetInfo(Task2Handle,taskstate,pdTRUE,eInvalid);
	
	HAL_UART_Transmit(&UART_HandInit,taskstate->pcTaskName,10,100);
	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
	
	HAL_UART_Transmit(&UART_HandInit,"\t",1,100);
	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
	
	sprintf(str,"%ld",taskstate->uxCurrentPriority);
	HAL_UART_Transmit(&UART_HandInit,str,1,100);
	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
	...
	sprintf(str,"%ld",taskstate->xTaskNumber);
	HAL_UART_Transmit(&UART_HandInit,str,1,100);
	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
	...
	sprintf(str,"%d",taskstate->eCurrentState);
	HAL_UART_Transmit(&UART_HandInit,str,1,100);
	while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
