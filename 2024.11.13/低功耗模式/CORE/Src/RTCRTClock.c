...
RTC_AlarmTypeDef RTC_CLOCKAlarmInit;
uint8_t wasd;
typedef struct time{
	int year;
	int month;
...
//	RTC_RTClockHandInit.State=
	...
	if(HAL_RTCEx_BKUPRead(&RTC_RTClockHandInit,RTC_BKP_DR1)!=0X5555){
		TIME_Set(&t,2024,11,13,11,0,0,2);
		MYRTC_SetTime(t);
		HAL_RTCEx_BKUPWrite(&RTC_RTClockHandInit,RTC_BKP_DR1,0X5555);
	}
	
//	RTC_CLOCKAlarmInit.Alarm=RTC_ALARM_A;
//	RTC_CLOCKAlarmInit.AlarmTime.Hours=;
//	
//	HAL_RTC_SetAlarm(&RTC_RTClockHandInit,&RTC_CLOCKAlarmInit);
...
void RTC_Alarm_IRQHandler(void){
	HAL_RTC_AlarmIRQHandler(&RTC_RTClockHandInit);
}
