...
void FreeRtos_Init(void){
	...
	EventGroup_Test = xEventGroupCreate();												/*创建时间标志组*/
	if(EventGroup_Test!=NULL){
		HAL_UART_Transmit(&UART_HandInit,"事件标志组创建成功\r\n",22,2000);
...
void EntryTask2(void const * argument)
{
	...
	while(1)
	{
		Data = xEventGroupWaitBits(EventGroup_Test,
							...
							portMAX_DELAY);
		sprintf(str,"%u",Data);
		HAL_UART_Transmit(&UART_HandInit,"事件标志位：",12,2000);
		while(__HAL_UART_GET_FLAG(&UART_HandInit,UART_FLAG_TC)!=1);
